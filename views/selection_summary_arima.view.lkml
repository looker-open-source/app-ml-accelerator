  ...
  derived_table: {
    sql:  SELECT column_stats.column_name
          ...
          FROM (SELECT column_name,
                ...
                FROM (SELECT trim(column_name, '"') AS column_name
                      ...
                      FROM (SELECT REGEXP_REPLACE(TO_JSON_STRING(t), r'^{|}$', '') AS row_json
                            FROM `@{GCP_PROJECT}.@{BQML_MODEL_DATASET_NAME}.{% parameter selection_summary_arima.input_data_view_name %}` AS t ) table_AS_json
          ...
          INNER JOIN (SELECT table_catalog
                      ...
                      WHERE table_name = '{% parameter selection_summary_arima.input_data_view_name %}'
                      ) column_metadata
